...
function ReportsTab() {
  ...
  const load = async () => {
    try {
      const res = await adminApi.getReports(null, 0, 50);
      setReports(res.data.data.content || []);
    } catch {} finally { setLoading(false); }
  };

  const handleResolve = async (id) => {
    const takeAction = !!actionCheck[id];
    const confirmMsg = takeAction
      ? '신고를 처리하고 제재 조치를 적용합니다.\n(상품 신고: 상품 취소 / 회원 신고: 계정 정지)\n\n계속하시겠습니까?'
      : '신고를 처리 완료로 변경합니다. 계속하시겠습니까?';
    if (!window.confirm(confirmMsg)) return;
    try {
      await adminApi.resolveReport(id, noteInput[id] || '', takeAction);
      alert(takeAction ? '신고 처리 + 제재 조치 완료' : '신고 처리 완료');
      load();
    } catch (e) { alert(e.response?.data?.message || '처리에 실패했습니다'); }
  };

  const handleDismiss = async (id) => {
    if (!window.confirm('이 신고를 기각하시겠습니까?\n(문제 없음으로 판단)')) return;
    try {
      await adminApi.dismissReport(id, noteInput[id] || '');
      alert('신고가 기각되었습니다');
      load();
    } catch (e) { alert(e.response?.data?.message || '처리에 실패했습니다'); }
  };

  const statusLabel = { PENDING: '미처리', RESOLVED: '처리완료', DISMISSED: '기각' };
  const targetLabel = { PRODUCT: '상품', MEMBER: '회원', REVIEW: '리뷰' };
  ...
  return (
    <CardList>
      ...
      {reports.map((r) => (
        <ReportCard key={r.id}>
          <ReportHeader>
            <Badge variant={r.status === 'PENDING' ? 'trading' : r.status === 'RESOLVED' ? 'active' : 'cancelled'}>
          ...
          <ReportBody>
            <ReportInfoGrid>
              <ReportInfoItem>
                ...
                <ReportInfoValue>{r.reporter?.nickname || `회원 #${r.reporter?.id}`}</ReportInfoValue>
              </ReportInfoItem>
              <ReportInfoItem>
                <ReportInfoLabel>신고 대상</ReportInfoLabel>
                <ReportInfoValue>{targetLabel[r.targetType] || r.targetType} #{r.targetId}</ReportInfoValue>
              </ReportInfoItem>
              <ReportInfoItem>
                <ReportInfoLabel>신고 사유</ReportInfoLabel>
                <ReportInfoValue style={{color: 'var(--danger)', fontWeight: 600}}>{r.reason}</ReportInfoValue>
              </ReportInfoItem>
            </ReportInfoGrid>

            {r.detail && (
              <ReportDetailBox>
                <ReportInfoLabel>상세 내용</ReportInfoLabel>
                <p>{r.detail}</p>
              </ReportDetailBox>
            )}

            {r.adminNote && (
              <ReportDetailBox style={{background: '#DBEAFE'}}>
                <ReportInfoLabel>관리자 메모</ReportInfoLabel>
                <p>{r.adminNote}</p>
              </ReportDetailBox>
            )}
            ...
            <ReportActions>
              ...
              <ActionCheckRow>
                <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, cursor: 'pointer' }}>
                  <input
                    type="checkbox"
                    checked={!!actionCheck[r.id]}
                    onChange={(e) => setActionCheck({ ...actionCheck, [r.id]: e.target.checked })}
                  />
                  <strong style={{color: 'var(--danger)'}}>제재 조치</strong>
                  <span style={{color: 'var(--text-tertiary)'}}>
                    ({r.targetType === 'PRODUCT' ? '상품 경매 취소' : r.targetType === 'MEMBER' ? '계정 정지' : '조치'})
                  </span>
                </label>
              </ActionCheckRow>
              <BtnRow>
                <SmallBtn onClick={() => handleResolve(r.id)}>
                  <FiCheck size={14} /> {actionCheck[r.id] ? '처리 + 제재' : '처리 완료'}
                </SmallBtn>
                <SmallBtn $danger onClick={() => handleDismiss(r.id)}>
                  <FiX size={14} /> 기각 (문제없음)
                </SmallBtn>
              </BtnRow>
            </ReportActions>
...
const ReportInfoLabel = styled.p`font-size: 12px; font-weight: 600; color: var(--text-tertiary); margin-bottom: 4px;`;
const ReportInfoValue = styled.p`font-size: 14px; font-weight: 500;`;
...
`;
const ActionCheckRow = styled.div`
  padding: 8px 0;
`;
