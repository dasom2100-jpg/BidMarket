...
public class AdminController {
    ...
    public ResponseEntity<ApiResponse<Void>> resolveReport(
            @PathVariable Long reportId, @RequestBody Map<String, Object> body) {
        String adminNote = (String) body.get("adminNote");
        boolean takeAction = Boolean.TRUE.equals(body.get("takeAction"));
        adminService.resolveReport(reportId, adminNote, false, takeAction);
        String msg = takeAction ? "신고가 처리되고 제재 조치가 적용되었습니다" : "신고가 처리되었습니다";
    ...
    public ResponseEntity<ApiResponse<Void>> dismissReport(
            @PathVariable Long reportId, @RequestBody Map<String, Object> body) {
        String adminNote = (String) body.get("adminNote");
        adminService.resolveReport(reportId, adminNote, true, false);
    ...
    public ResponseEntity<ApiResponse<Void>> createNotice(
        ...
        adminService.createNotice(adminId,
                (String) body.get("title"),
                (String) body.get("content"),
                Boolean.TRUE.equals(body.get("isPinned")));
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(ApiResponse.success("공지사항이 등록되었습니다"));
    }
